   ...
   const getAvailableSlots = async () => {
      ...
      for (let i = 0; i < 7; i++) {
         ...
         while (currentDate < endTime) {
            ...
            let month = currentDate.getMonth() + 1; // Correct month indexing
      ...
      try {
        ...
        const {data} = await axios.post(backendUrl + '/api/user/book-appointment', {docId,slotDate,slotTime},{headers:{token}})
         ...
         if (error.response && error.response.data) {
            toast.error(error.response.data.message); // Show the custom message from the backend
         } else {
      ...
      {docInfo && (
         ...
         <div className='flex flex-col sm:flex-row gap-4 '>
            ...
            <div className='flex-1 border border-gray-400 rounded-lg p-8 py-7  bg-white mx-2 sm:mx-0 mt-[-80px] sm:mt-0 '>
               ...
               <img className='w-5' src={assets.verified_icon} alt="error " />
               </p>
               ...
               <div>
                  <p className='flex items-center gap-1 text-sm font-medium text-gray-900 mt-3  '>About <img src={assets.info_icon} alt="error" /></p>
